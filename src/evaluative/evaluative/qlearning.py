...
class Q_learning:
    def __init__(self, alpha=0.5, gamma=0, eps=0, nEpisodes=30, timeout=20, width=5, action_space=6):
        self.alpha = alpha
        self.gamma = gamma
        self.eps = eps
    ...
    def update(self, x, u, feedback):
        action_value = self.w[u, x]
        self.w[u][x] += self.alpha * (feedback - action_value)
